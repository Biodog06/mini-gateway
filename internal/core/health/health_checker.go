...
import (
	...
	"github.com/DoraZa/mini-gateway/pkg/cache" // 引入 Redis 包
...
func InitHealthChecker(cfg *config.Config) *HealthChecker {
	...
	if cache.Client == nil {
		logger.Warn("Redis client not initialized, disabling health stats storage and heartbeat")
		once.Do(func() {
...
func (h *HealthChecker) clearRedisKeys() error {
	...
	for _, pattern := range patterns {
		if cache.Client == nil {
			logger.Warn("Redis client not initialized, skipping key cleanup",
