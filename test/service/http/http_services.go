...
func startUserService3(wg *sync.WaitGroup) {
	defer wg.Done()
	mux := http.NewServeMux()
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintf(w, `{"service": "user", "path": "%s", "port": "8384"}`, r.URL.Path)
	})
	mux.HandleFunc("/status", func(w http.ResponseWriter, r *http.Request) {
		log.Println("Received health check for user service")
		fmt.Fprintf(w, `{"health":"ok"}`)
	})
	server := &http.Server{Addr: ":8384", Handler: mux}
	fmt.Println("User Service 3 running on :8384")
	if err := server.ListenAndServe(); err != nil {
		fmt.Println("User Service 3 failed:", err)
	}
}
